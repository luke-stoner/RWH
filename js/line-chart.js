...
d3.csv("data/labeled.csv", (row) => {
  ...
  document
    .getElementById("line-stat-selection")
  ...
  function updateVisualization() {
        ...
        const weeklyData = d3.timeWeek
          ...
          .map((week) => {
            const filtered = values.filter(
              (d) => d.date >= week && d.date < d3.timeWeek.offset(week, 2),
            );
    ...
    xScale.domain([
      d3.min(aggregatedData, (d) => d3.min(d.values, (v) => v.date)),
      d3.max(aggregatedData, (d) => d3.max(d.values, (v) => v.date)),
    ]);
    yScale.domain([
      0,
    ...
    const points = svg.selectAll(".point-group").data(aggregatedData);
    ...
    const circles = allPoints.selectAll("circle").data((d) => d.values);
