...
d3.csv("data/labeled.csv", (row) => {
  ...
  const sbf_svg = d3
    ...
    .attr(
      "transform",
      "translate(" + sbfMargin.left + "," + sbfMargin.top + ")",
    );
    ...
    const sentimentData = d3
      ...
      .map((d) => ({
        ...
        photo:
          ...
          ".png",
        party: filteredData.find(
          (r) => r.first_name + " " + r.last_name === d[0],
        ).party,
      }));
  ...
  function update_network_visualization() {
    ...
    const filteredData = filtered_date_data.filter((entry) =>
      networksToInclude.includes(entry.network),
    );
