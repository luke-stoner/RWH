...
// let margin_bump = { top: 40, right: 120, bottom: 70, left: 60 }; // Increased right margin
...
//     .select("#bump-chart-area")
//     .append("svg")
//     .attr("width", width_bump + margin_bump.left + margin_bump.right)
//     .attr("height", height_bump + margin_bump.top + margin_bump.bottom)
//     .append("g")
//     .attr("transform", "translate(" + margin_bump.left + "," + margin_bump.top + ")");
// format date
...
d3.csv("data/labeled.csv", row => {
    ...
    function updateVisualization() {
        ...
        // Define time format for month/day

        // // Inside your updateVisualization function, modify the xAxis transition
        // xAxis.transition()
        //     .duration(500)
        //     .call(d3.axisBottom(xScale)
        //         .tickFormat(formatDateToMonthDay)
...
// //     .attr("transform", "translate(" + margin_bump.left + "," + margin_bump.top + ")");
//
// function highlightLine_bump(candidateName) {
//     svg_bump.selectAll('.line, .datapoint').classed('dimmed', true); // Dim all lines and points
//     svg_bump.selectAll('.line-group').filter(d => d.candidate === candidateName)
...
// }
//
// function restoreLines_bump() {
//     svg_bump.selectAll('.line, .datapoint').classed('highlighted', false).classed('dimmed', false);
...
//     function updateVisualization() {
//
//         // Mouseover and Mouseout event listeners for lines, circles, and images
//         svg_bump.selectAll('.line, .datapoint, .datapoint-image')
//             .on('mouseover', (event, d) => {
//                 highlightLine_bump(d.candidate);
//             })
//             .on('mouseout', restoreLines_bump);
//         // Get the selected attribute from the dropdown
//         let column = document.getElementById("bump-chart-selection").value;
//
//         // Filter data to only include specified candidates consistently in the top ten of mentions
...
//             .attr("r", imageSize / 1.5) // Adjust the radius to accommodate the image size and padding
//             .style("fill", (d) => d3.color(colorScale(d.candidate)).copy({opacity: 0.4}))
//             .on('mouseover', (event, d) => highlightLine_bump(d.candidate))
//             .on('mouseout', restoreLines_bump);
//
//         // Update circles for data points to use candidate images
//         const images = allPoints
...
//     bump_leftcol_pxls = parseFloat(d3.select('#bumpchartcolumn').style('width'));
//     width_bump = bump_leftcol_pxls - margin_bump.left - margin_bump.right;
//     svg_bump.attr("width", width_bump + margin_bump.left + margin_bump.right);
//     updateVisualization();
// });
//
// d3.select(window).on('resize', function(){
