...
let svg_bump = d3
  ...
  .attr(
    "transform",
    "translate(" + margin_bump.left + "," + margin_bump.top + ")",
  );
...
d3.csv("data/labeled.csv", (row) => {
  ...
  const xAxis = svg_bump
    .append("g")
    .attr("transform", `translate(0,${height_bump + 30})`);
  ...
  document
    .getElementById("bump-chart-selection")
  ...
  function updateVisualization() {
    ...
    let filtered_data = data.filter((d) =>
      selected_candidates.includes(d.last_name),
    );
        ...
        const weeklyData = d3.timeWeek
          ...
          .map((week) => {
            const filtered = values.filter(
              (d) =>
                d.date >= week && d.date < d3.timeWeek.offset(week, num_weeks),
            );
    ...
    const nestedRankedData = Array.from(
      d3.group(rankedData, (d) => d.candidate),
      ([key, values]) => {
        return {
          candidate: key,
          values: values,
    ...
    xAxis
      .transition()
      .duration(500)
      .call(d3.axisBottom(xScale).ticks(d3.timeWeek.every(num_weeks)));
    yAxis
      .transition()
      .duration(500)
      .call(
    ...
    const circles = allPoints.selectAll(".circles").data((d) => d.values); // Ensure data is correctly bound using 'values'
    ...
    const images = allPoints.selectAll("image").data((d) => d.values); // Ensure data is correctly bound using 'values'
    ...
    const clipPaths = allPoints.selectAll(".clip-path").data((d) => d.values);
    ...
    images
      ...
      .attr("xlink:href", (d) => {
        ...
        const candidate = nestedRankedData.find(
          (candidate) => candidate.candidate === d.candidate,
        );
      ...
      .on("mouseover", function (event, d) {
        ...
        svg_bump.selectAll(".line").style("opacity", function (lineData) {
          ...
          tooltip
            .html(
              `<div style="text-align: center; font-weight: bold;">
                               ...
                               Positive Mentions: ${formatAsPercentage(
                                 d.label,
                               )}`,
            )
            .style("left", event.pageX + 12 + "px")
      ...
      .on("mouseout", function (d) {
        ...
        svg_bump.selectAll(".line").style("opacity", 1);

        // Restore opacity of all circles' strokes on mouseout
        svg_bump.selectAll(".circles").style("stroke-opacity", 1);

        // Restore opacity of all circles' strokes on mouseout
        svg_bump.selectAll(".datapoint-image").style("opacity", 1);
    ...
    nestedRankedData.forEach((candidateData) => {
      const lastDataPoint =
        candidateData.values[candidateData.values.length - 1];
      svg_bump
        .append("text")
